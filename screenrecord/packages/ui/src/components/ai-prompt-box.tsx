...
const StyleInjector = () => {
  React.useEffect(() => {
    const styleSheet = document.createElement("style");
    styleSheet.innerText = styles;
    styleSheet.setAttribute('data-ai-prompt-styles', 'true');
    document.head.appendChild(styleSheet);
    
    return () => {
      const existingStyle = document.querySelector('[data-ai-prompt-styles="true"]');
      if (existingStyle) {
    ...
    >
      ...
      <div className="w-full h-10 flex items-center justify-center gap-0.5 px-4">
        {[...Array(visualizerBars)].map((_, i) => (
          <div
            key={i}
            className="w-0.5 rounded-full bg-foreground animate-pulse"
            style={{
      ...
      >
                ...
                {file.type.startsWith("image/") && filePreviews[file.name] && (
                  <div
                    ...
                    onClick={() => {
                      ...
                      if (previewUrl) openImageModal(previewUrl);
                    }}
                  >
                    <img
                    ...
                    <button
                      ...
                      className="absolute top-1 right-1 rounded-full bg-background/90 p-0.5 opacity-100 transition-opacity"
                    >
        ...
        <PromptInputActions className="flex items-center justify-between gap-2 p-0 pt-2">
          ...
          >
            ...
            >
              {isLoading ? (
                <Square className="size-4 fill-foreground animate-pulse" />
              ) : isRecording ? (
                <StopCircle className="size-5 text-destructive" />
              ) : hasContent ? (
                <ArrowUp className="size-4 text-primary-foreground" />
              ) : (
                <Mic className="size-5 text-muted-foreground transition-colors" />
              )}
            </Button>
          </PromptInputAction>
        </PromptInputActions>
