...
# Bundled service startup script for ScreenRecord Tracker
...
RESOLVED_INFLUX_TOKEN="screenrecord-admin-token-change-in-production"
...
if wait_for_service "InfluxDB" 8086 30 "http://localhost:8086/health"; then
    ...
    if [ "$NEEDS_SETUP" = "true" ]; then
        ...
        SETUP_ORG="screenrecord-org"
        SETUP_BUCKET="screenrecord-metrics"
        SETUP_TOKEN="screenrecord-admin-token-change-in-production"
    ...
    BUCKETS_HTTP=$(curl -s -o /dev/null -w "%{http_code}" -H "Authorization: Token $RESOLVED_INFLUX_TOKEN" "http://localhost:8086/api/v2/buckets?org=screenrecord-org" 2>&1)
...
SERVER_HOST=0.0.0.0 \
SERVER_PORT=8080 \
JWT_SECRET=screenrecord-bundled-secret-key \
...
OPENAI_API_KEY="" \
GEMINI_API_KEY_FILE="$APP_DATA_DIR/gemini_api_key.txt" \
