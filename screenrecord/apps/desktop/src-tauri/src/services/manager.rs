...
pub async fn start_influxdb(app_handle: AppHandle) -> Result<(), String> {
    ...
    if needs_setup {
        ...
        cmd.env("DOCKER_INFLUXDB_INIT_ORG", "screenrecord-org");
        cmd.env("DOCKER_INFLUXDB_INIT_BUCKET", "screenrecord-metrics");
        cmd.env("DOCKER_INFLUXDB_INIT_ADMIN_TOKEN", "screenrecord-admin-token-change-in-production");
    }
...
pub async fn start_collector(app_handle: AppHandle) -> Result<(), String> {
    ...
    cmd.env("JWT_SECRET", "screenrecord-bundled-secret-key");
    cmd.env("INFLUXDB2_URL", "http://localhost:8086");
    cmd.env("INFLUXDB2_TOKEN", "screenrecord-admin-token-change-in-production");
    cmd.env("INFLUXDB2_ORG", "screenrecord-org");
    cmd.env("INFLUXDB2_BUCKET", "screenrecord-metrics");
